...
val StudentApp = staticCompositionLocalOf<StudentAppContext> {
    error("No AppContext provided")
}

@Composable
fun rememberStudentApp(
    ...
    val user = viewModel { UserContext(auth, dbSyncer, accountLogicSyncer, UserType.Student) }
    val log = viewModel { AlarmContext(user, logDbSyncer, logLogicSyncer) }
    return remember(user, log) { StudentAppContext(user, log) }
}

package org.example.pipe2.ui.pages.loggingIn
...
fun LoginPage(
    ...
    val userContext: UserContext = SharedApp.current.user
    val context: LogInPageContext = viewModel { LogInPageContext(userContext) }
    ...
    Column(
        ...
        when (LocalAppConfig.current.type) {
            UserType.Student -> devSignIn("student", context)
            UserType.Warden -> devSignIn("warden", context)
            else -> {}
        }
...
private fun DevFirebaseErrorHandler() {
    val log = SharedApp.current.alarm
    if (log.error != null) {
        Column(
